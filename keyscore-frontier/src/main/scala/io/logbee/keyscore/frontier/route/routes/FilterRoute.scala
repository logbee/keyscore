...
import io.logbee.keyscore.commons.metrics.{ScrapeFilterMetricRequest, ScrapedFilterMetricResponse, ScrapedFilterMetricResponseFailure}
      ...
      authorize { token =>
        pathPrefix(JavaUUID) { filterId =>
          path("pause") {
            ...
            path("scrape") {
              get {
                onSuccess(metricsManager ? ScrapeFilterMetricRequest(filterId)) {
                  case ScrapedFilterMetricResponse(id, metricsCollection) => complete(StatusCodes.OK, metricsCollection)
                  case ScrapedFilterMetricResponseFailure(id) => complete(StatusCodes.NotFound, id)
                  case _ => complete(StatusCodes.InternalServerError)
                }
              }
            }
