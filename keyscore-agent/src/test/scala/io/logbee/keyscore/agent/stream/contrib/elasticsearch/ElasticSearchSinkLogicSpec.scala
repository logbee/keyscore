...
import io.logbee.keyscore.agent.stream._
import io.logbee.keyscore.agent.stream.stage.{SinkStage, StageContext}
import io.logbee.keyscore.model.Dataset
import io.logbee.keyscore.model.filter.{FilterConfiguration, IntParameter, TextParameter}
...
class ElasticSearchSinkLogicSpec  extends WordSpec with Matchers with ScalaFutures with MockFactory with TestSystemWithMaterializerAndExecutionContext {
  ...
  "A ElasticSearchSink" should {
    ...
    "do some thing" in {
      ...
      val configuration = FilterConfiguration(UUID.randomUUID(), "", List(TextParameter("host", "localhost"), IntParameter("port", 9200), TextParameter("index", "test")))
