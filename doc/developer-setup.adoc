...
* Enable auto-import and
* use the recommended gradle wrapper.
...
.Gradle fails with with reading the docker config.json

Just remove the config, since the gradle docker plugin can not deal with this config file.
