...
4. When the import finishes run the the following gradle task from the root of the project to generate all source files:
----
...
[CAUTION]
====
Gradle's parallel execution can lead to failing builds. E.g. there are known problems when running `npm` tasks of the keyscore-manager.
Disable parallel exectuin by setting `org.gradle.parallel=false` in `~/.gradle/gradle.properties`.
====
