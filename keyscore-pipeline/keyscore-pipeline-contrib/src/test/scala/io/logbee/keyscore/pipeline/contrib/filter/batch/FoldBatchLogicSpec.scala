...
class FoldBatchLogicSpec extends FreeSpec with ScalaFutures with Matchers with TestSystemWithMaterializerAndExecutionContext {
  ...
  "A FoldBatchLogic" - {
    ...
    "when processing left-to-right" - {
      ...
      "should overwrite the first with the last" in new TestStreamWithSourceAndSink(configuration, classOf[FoldBatchLogic]) {
    ...
    "when processing right-to-left" - {
      ...
      "should overwrite the last with the first" in new TestStreamWithSourceAndSink(configuration, classOf[FoldBatchLogic]) {
