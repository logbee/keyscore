...
class ReadSchedulerSpec extends FreeSpec with Matchers with MockFactory with BeforeAndAfter {
  ...
  "A ReadScheduler should" - {
    ...
    new ReadSchedulerSetup with PersistenceContextWithoutTimestamp {
      ...
      (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile, startPos=0, endPos=logFile.length, logFile.lastModified))
    ...
    new ReadSchedulerSetup with RotateFiles {
      ...
      inSequence {
        ...
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile4_ModifiedBeforePreviousReadTimestamp, startPos=previousReadPosition, endPos=logFile4_ModifiedBeforePreviousReadTimestamp.length, logFile4_ModifiedBeforePreviousReadTimestamp.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile3_ModifiedAfterPreviousReadTimestamp, startPos=0, endPos=logFile3_ModifiedAfterPreviousReadTimestamp.length, logFile3_ModifiedAfterPreviousReadTimestamp.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile2, startPos=0, endPos=logFile2.length, logFile2.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile1, startPos=0, endPos=logFile1.length, logFile1.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile, startPos=0, endPos=logFile.length, logFile.lastModified))
      }
    ...
    new ReadSchedulerSetup with RotateFiles {
      ...
      inSequence {
        ...
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile1, startPos=0, endPos=logFile1Data.getBytes.length, logFile1.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile, startPos=0, endPos=logFileData.getBytes.length,  logFile.lastModified))
      }
    ...
    new ReadSchedulerSetup with PersistenceContextWithoutTimestamp {
      ...
      inSequence {
        ...
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile, startPos=0, endPos=previousReadPosition, logFile.lastModified))
        readScheduler.fileModified()
        ...
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile, startPos=previousReadPosition, endPos=logFile.length, logFile.lastModified))
        readScheduler.fileModified()
      }
    }
    ...
    new ReadSchedulerSetup {
      ...
      inSequence {
        ...
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile, startPos=previousReadPosition, endPos=logFileData.getBytes.length, logFile.lastModified))
      }
    ...
    new ReadSchedulerSetup with RotateFiles {
      ...
      inSequence {
        ...
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile4_ModifiedBeforePreviousReadTimestamp, startPos=0, endPos=logFile4_ModifiedBeforePreviousReadTimestamp.length, logFile4_ModifiedBeforePreviousReadTimestamp.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile3_ModifiedAfterPreviousReadTimestamp, startPos=0, endPos=logFile3_ModifiedAfterPreviousReadTimestamp.length, logFile3_ModifiedAfterPreviousReadTimestamp.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile2, startPos=0, endPos=logFile2.length, logFile2.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile1, startPos=0, endPos=logFile1.length, logFile1.lastModified))
        (readSchedule.queue _).expects(ReadScheduleItem(logFile, logFile, startPos=0, endPos=logFile.length, logFile.lastModified))
