    ...
    "should read the contents of" - {
      "its file" - {
        "line by line, if line-wise reading is active," - {
          ...
          "if the file contains only one line of text" in new LogFile {
            ...
            fileReader.fileModified(mockCallback, ReadScheduleItem(logFile, logFile, 0, logFile.length, logFile.lastModified))
          }
      ...
      "its file as well as its rotated files, if rotation is active" - {
        "line by line, if line-wise reading is active" in new RotateFiles {
          ...
          fileReader.fileModified(mockCallback, ReadScheduleItem(logFile, logFile3_ModifiedAfterPreviousReadTimestamp, previousReadPosition, logFile3_ModifiedAfterPreviousReadTimestamp.length, logFile3_ModifiedAfterPreviousReadTimestamp.lastModified))
          fileReader.fileModified(mockCallback, ReadScheduleItem(logFile, logFile2, 0, logFile2.length, logFile2.lastModified))
          fileReader.fileModified(mockCallback, ReadScheduleItem(logFile, logFile1, 0, logFile1.length, logFile1.lastModified))
          fileReader.fileModified(mockCallback, ReadScheduleItem(logFile, logFile, 0, logFile.length, logFile.lastModified))
        }
        ...
        bufferSizesToTest.foreach { case TestCase(bufferSize, description) =>
          ...
          "with buffer size " + bufferSize + " bytes, which is " + description in new LogFile {
            ...
            filesToRead.foreach { file =>
              ...
              fileReader.fileModified(string => calledBackString += string, ReadScheduleItem(logFile, logFile, 0, file.length, file.lastModified))
            }
...
//          
//          fileReader.fileModified((_: String) => (), ReadScheduleItem(logFile, logFile, 0, logFile.length, logFile.lastModified))
//        }
//
