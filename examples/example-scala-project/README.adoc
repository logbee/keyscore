...
[[example-scala-project-source-tree]]
.The directory structure of the example-scala-project.
[source]
----
...
project with gradle. Read the <<example-scala-project-gradle>> section for more details about the `build.gradle`.
...
A very important resource is the `MANIFEST.MF` file. Read more about it in the <<example-scala-project-jar-manifest>>
section.
...
[[example-scala-project-artifact-tree]]
.Structure of the resulting artifact.
[source]
----
example-scala-project.jar
├── io
│   └── logbee
│       └── keyscore
│           └── example
...
[[example-scala-project-jar-manifest]]
==== Manifest ====
...
[[example-scala-project-manfiest]]
[source]
...
[[example-scala-project-localization]]
==== Localization ====

[[example-scala-project-gradle]]
...
You can jump to jump to <<scala-example-project-gradle-tasks>> if your are only interested in how to run the relevant
Gradle tasks.
...
After an successful build you can publish your artifact to an maven repository by running the following task:
...
To test your code in the keyscore ecosystem, run the following task:

[source,bash]
----
$ gradle runContainers
----

When you finished testing - or before running a new version of your code - you have to run the following task:
