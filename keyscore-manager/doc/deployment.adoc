...
docker run --publish 8080:80 \
           quay.io/logbee/keyscore-manager:0.3.0-alpha.6
----
...
2.  Copy the web-content of the zip-distribution into the Docker image. In the example above everything is copied
    to `/usr/share/nginx/html/`.

3.  At the end, specify the command to run the web-server.
